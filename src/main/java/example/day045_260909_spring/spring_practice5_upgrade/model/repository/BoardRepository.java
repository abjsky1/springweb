...
//  게시물 목록 조회 기능
    ...
    List<Map<BoardDto,Object>> boardFindAllQuery();
    // 질문 password 필요한가?

// +게시물 목록 개별 조회 기능
    @Query(value = "select * from board where id = :id" , nativeQuery = true)
    Map<BoardDto,Object> boardFindDetailQuery(Integer id);

    @Query( value = "select * from comment where board_id = :id" , nativeQuery = true)
    List<Map<CommentDto,Object>> commentQuery( Object id );
}
